...
## 2024-05-24 - [Avoid Chained Filters]
**Learning:** Chaining multiple `.filter()` calls (e.g., `array.filter(A).filter(B).filter(C)`) creates multiple intermediate array allocations and causes redundant iterations (O(N * K)).
**Action:** Combine multiple conditions into a single `.filter()` pass using logical `&&` to reduce iterations to O(N), minimize memory allocations, and leverage short-circuit evaluation.
