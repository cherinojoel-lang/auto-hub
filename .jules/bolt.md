...
## 2024-07-21 - Optimize chained array filters
**Learning:** Chaining multiple `.filter()` calls creates intermediate arrays and forces multiple full passes over the data.
**Action:** Combine multiple conditions into a single `.filter()` pass using logical `&&` to reduce iterations and memory allocations.
