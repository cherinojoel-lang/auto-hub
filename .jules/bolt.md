...
## 2024-05-24 - Optimize O(N) array iterations over `.filter(...).slice(0, N)`
**Learning:** Found multiple places using `.filter(...).slice(0, N)` pattern on arrays, which filters the entire array first (O(N) operations and memory allocation for the filtered array) before discarding most elements via `slice`.
**Action:** Replaced `.filter(...).slice(0, N)` with a single-pass `for...of` loop or equivalent that breaks early once `N` elements are collected.
