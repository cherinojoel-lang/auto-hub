...
## 2024-05-15 - [WeakMap Caching for getTransmission]
**Learning:** The `getTransmission` function was executing regular expressions (`pattern.test`) repeatedly without caching. Using `WeakMap` prevents memory leaks while drastically improving performance by returning cached results instantly in React render loops.
**Action:** Use `WeakMap` for all string/object derivation on stable entity objects in render loops.
