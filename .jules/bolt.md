...
## 2024-07-17 - [O(N) filter().slice() Anti-pattern]
**Learning:** The codebase frequently relies on `.filter(...).slice(0, N)` over global collections to render top or similar vehicles. This forces a full O(N) array traversal and unnecessary memory allocation before taking the small slice.
**Action:** Replace `.filter(...).slice(0, N)` patterns over collections with a single-pass `for...of` loop that breaks early once `N` elements are collected to minimize iterations and reduce memory overhead.
