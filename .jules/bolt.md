...
## 2026-08-29 - Prevent VDOM Allocations for Global Arrays
**Learning:** Repeatedly iterating over static global arrays or performing unmemoized slice operations directly inside render bodies causes unnecessary memory allocations and VDOM comparison cycles on every re-render.
**Action:** Always safely isolate global derived iterators using React.useMemo(() => globalArray.slice(0, n), []) with stable dependencies to prevent unnecessary waterfall updates.
