...
## 2025-03-01 - Optimize O(N) array iteration with early-exit loops
**Learning:** Using `.filter(...).slice(0, N)` on large arrays iterates through the entire array and creates an intermediate array in memory, causing unnecessary overhead.
**Action:** Replace `.filter(...).slice(0, N)` patterns with a single-pass `for...of` loop that breaks early once `N` elements are collected.
