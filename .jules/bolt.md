...
## 2024-05-15 - [Single-Pass Array Filtering]
**Learning:** Chaining multiple `.filter()` calls on an array creates intermediate array allocations and causes multiple iterations over the dataset (O(N*M)).
**Action:** Always combine sequential `.filter()` conditions into a single `.filter()` pass using logical `&&` to optimize performance and reduce memory overhead.
