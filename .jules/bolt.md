...
## 2024-05-24 - [Optimize O(N) filter.slice array iterations]
**Learning:** Chaining `.filter(condition).slice(0, N)` on large arrays iterates through the entire collection and allocates an intermediate array in memory before truncating it.
**Action:** Replace `.filter(...).slice(0, N)` with a single-pass `for...of` loop that pushes to a result array and breaks early once `N` elements are collected to avoid unnecessary iteration and memory allocation.
