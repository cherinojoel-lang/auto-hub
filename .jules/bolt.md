...
## 2024-08-16 - Replace filter().slice() with single-pass loop
**Learning:** Found an O(N) traversal pattern where `array.filter(...).slice(0, N)` iterates through the entire dataset and allocates a new array for all matched elements before truncating. This is inefficient for large datasets or frequent renders.
**Action:** Replace `array.filter(condition).slice(0, N)` with a single-pass `for...of` loop that breaks early after collecting `N` elements, preventing full array traversals and excess memory allocation.
