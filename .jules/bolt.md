...
## 2024-07-15 - Eliminate Chained Filter Iterations
**Learning:** Chaining multiple `.filter()` methods over arrays (like in vehicle filtering) causes an O(N) iteration and a new array memory allocation for every single `.filter()` step in the chain.
**Action:** Combine multiple array filtering conditions using logical `&&` inside a single `.filter()` pass to minimize iterations and reduce intermediate memory footprint.
