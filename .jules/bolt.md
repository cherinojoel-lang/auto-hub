...
## 2024-05-15 - [Array Optimization]
**Learning:** Using `.filter(...).slice(0, N)` over collections triggers O(N) full array iterations and allocates intermediate memory arrays unnecessarily.
**Action:** Always optimize this pattern by replacing it with a single-pass `for...of` loop that breaks early once `N` elements are collected.
