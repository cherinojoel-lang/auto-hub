...
## 2026-06-23 - [Middleware Object.entries Optimization]
**Learning:** In Cloudflare/Astro middleware (`src/middleware.ts`), dynamically generating arrays from constant config objects using `Object.entries()` inside the high-throughput `onRequest` handler introduces redundant memory allocations and CPU overhead on every incoming HTTP request.
**Action:** Always hoist computations of static or constant data (like `Object.entries(SECURITY_HEADERS)`) to the module scope so they are evaluated only once at startup rather than repeatedly during runtime.
