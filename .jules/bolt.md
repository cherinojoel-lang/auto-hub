...
## 2024-05-27 - [Combine Filter Conditions]
**Learning:** Chaining multiple `.filter()` calls on arrays iterates over the array multiple times, which allocates intermediate arrays and negatively impacts performance, especially for large datasets.
**Action:** Always combine multiple filter conditions into a single `.filter()` pass using logical `&&` to reduce iterations and memory allocations.
