...
**Learning:** Chaining `.filter().slice(0, N)` on large arrays iterates over the entire array to create a new filtered array before taking the first N elements, which is O(N) and causes unnecessary memory allocation.
**Action:** Replace `.filter().slice(0, N)` with a single-pass `for...of` loop that breaks early once N elements are collected to optimize performance and reduce memory allocations.
