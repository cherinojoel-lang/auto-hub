...
## 2024-05-15 - [deriveFeatures Performance Optimization]
**Learning:** The `deriveFeatures` utility function in `vehicleFeatures.ts` was previously optimized to cache features, but `getVehicleImageCount` and `getTransmission` were still executing expensive string and regex operations on every render without caching.
**Action:** Applied `WeakMap` caching to `getVehicleImageCount` and `getTransmission` to ensure O(1) lookups and eliminate redundant computations during re-renders, further reducing block time.
