...
## 2024-05-15 - [Combine chained Array.filter calls]
**Learning:** Chaining multiple `.filter()` calls on an array creates intermediate arrays and forces multiple full iterations over the dataset, leading to unnecessary memory allocations and degraded performance, especially on large lists.
**Action:** Combine multiple filtering conditions into a single `.filter()` pass using logical `&&` to optimize array iteration and minimize memory allocations.
