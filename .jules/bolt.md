...
## 2024-05-15 - [Chained Array Filters]
**Learning:** Chaining multiple `.filter()` calls on an array creates intermediate arrays and forces multiple iterations over the dataset, leading to unnecessary memory allocation and processing overhead.
**Action:** Always combine multiple filter conditions using logical `&&` within a single `.filter()` pass to ensure optimal O(n) performance and avoid intermediate allocations.
