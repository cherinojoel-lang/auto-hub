...
## 2024-05-15 - [Early Return on Filter/Slice Iterations]
**Learning:** The `.filter(...).slice(0, N)` pattern performs an O(N) operation by iterating over the entire array before truncating. This wastes memory allocations and processing time for large collections.
**Action:** Replace `.filter(...).slice(0, N)` on large collections with a single-pass `for...of` loop that breaks early once `N` matching elements are collected, avoiding unnecessary iterations and intermediate array creations.
