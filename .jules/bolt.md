...
## 2024-05-15 - [Combine filter calls]
**Learning:** Chained `.filter()` calls in JavaScript create intermediate arrays, which can cause memory allocations and impact performance when processing large datasets or when called frequently.
**Action:** Always combine multiple `.filter()` conditions into a single `.filter()` call using logical `&&` to prevent intermediate array allocations and leverage short-circuit evaluation.
