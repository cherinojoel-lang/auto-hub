...
## 2025-02-27 - [Optimize .filter().slice() Patterns]
**Learning:** Using `.filter(...).slice(0, N)` on large collections causes full O(N) iterations and unnecessary memory allocations for the intermediate filtered array.
**Action:** Always replace `.filter(...).slice(0, N)` with a single-pass `for...of` loop that pushes matching elements into a new array and breaks early once `N` elements are collected to minimize iterations and memory usage.
