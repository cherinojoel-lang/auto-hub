...
## 2025-07-02 - Derived State Performance Bottleneck
**Learning:** Found an anti-pattern in `VehiclesPage` and `VehicleDetailPage` where local static data (`vehiclesData`) was being loaded into component state via `useEffect()` *after* the initial mount. This caused an unnecessary waterfall render cycle (first render: empty state/loading spinner, second render: populated data) and worsened Largest Contentful Paint (LCP) despite data being synchronously available.
**Action:** When data is statically available or synchronously imported, compute derived state directly during render using `useMemo()` instead of managing separate `useState` variables triggered by `useEffect`. This prevents double-rendering and ensures content is present on the first mount. Ensure any potential mock errors (e.g. in tests) are caught gracefully within the `useMemo` block.
