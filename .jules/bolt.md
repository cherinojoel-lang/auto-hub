...
## 2024-05-15 - [Combine chained Array filters]
**Learning:** Chaining multiple `.filter()` calls iterates over the entire array multiple times and creates intermediate arrays, decreasing performance.
**Action:** Combine conditions into a single `.filter()` pass using logical `&&` to reduce array iterations and memory allocations.
