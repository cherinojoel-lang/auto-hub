...
## 2026-09-12 - Dynamic Status Messages
**Learning:** Dynamically appearing success or error messages (e.g., form submissions) need `aria-live="polite"` in addition to `role="status"` to reliably trigger screen reader announcements.
**Action:** Always pair `role="status"` with `aria-live="polite"` for dynamically rendered feedback elements.
