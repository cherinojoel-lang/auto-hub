...
## 2026-09-08 - Supplementary Form Input Accessibility
**Learning:** When a form field (like Kaufpreis) uses both a primary number input and a supplementary range slider under a single visual label, screen readers struggle to associate them correctly without explicit id/htmlFor and aria-label properties, and sliders often lack keyboard focus indicators.
**Action:** Always assign id and htmlFor exclusively to the primary input. Provide the supplementary input with an explicit aria-label and focus-visible styling classes for keyboard navigation.
