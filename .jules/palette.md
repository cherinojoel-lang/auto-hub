...
## 2026-09-22 - Accessible Custom Form Validation
**Learning:** Custom forms in this app's components (like ContactSection) use local state for errors but lack accessible bindings (aria-describedby, aria-invalid) and live regions for dynamic messages.
**Action:** Always explicitly link dynamic error messages to inputs using aria-describedby, mark inputs with aria-invalid="true", assign role="alert" to error elements, and ensure dynamic status messages have both role="status" and aria-live="polite".
