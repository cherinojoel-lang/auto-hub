...
## 2024-09-03 - Accessible Dual-Input Form Patterns
**Learning:** When multiple form inputs share a single visual label (like a number input and a supplementary range slider for the same value), assigning `id` and `htmlFor` must be done exclusively to the primary input to prevent screen reader confusion.
**Action:** Always provide the supplementary input with an explicit `aria-label` and ensure it has distinct `focus-visible` styling, as native range sliders often lack clear default focus indicators in styled forms.
