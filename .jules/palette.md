...
## 2024-07-31 - Character Counters on Forms
**Learning:** Adding large limits to textareas (like 2000 chars) requires visual character counters linked via `aria-describedby` and `aria-live="polite"` to prevent DoS risks and maintain accessibility without confusing screen readers.
**Action:** Always pair `maxLength` on large textareas with an accessible character counter using `aria-live="polite"` for feedback.
