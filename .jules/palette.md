...
## 2024-06-03 - Textarea Character Counters
**Learning:** Textareas with large `maxLength` limits (e.g., 2000 characters) silently stop accepting input when the limit is reached, which frustrates users who don't understand why they can't type anymore.
**Action:** Always pair large `maxLength` attributes on textareas with a visual character counter (e.g., `current / limit`) to provide clear feedback and improve user experience.
