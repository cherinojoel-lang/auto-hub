...
## 2026-09-26 - Explicit Form Label Associations
**Learning:** Even when inputs immediately follow labels visually, screen readers rely on explicit `htmlFor` and `id` linking to reliably announce the input's purpose. Missing these in custom forms is a common accessibility trap.
**Action:** Always add explicit `htmlFor` and `id` pairs to form elements, especially when not using a specialized form library that handles it automatically.
