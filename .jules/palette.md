...
## 2025-03-09 - Form Input Limits and Feedback
**Learning:** Applying `maxLength` attributes to `textarea` fields without visual feedback leads to a confusing user experience, as keystrokes are silently ignored once the limit is reached.
**Action:** Pair `maxLength` attributes (especially on large inputs like messages) with a visual character counter (e.g. `125 / 2000`) and appropriate ARIA live regions so users know exactly how much they can type.
