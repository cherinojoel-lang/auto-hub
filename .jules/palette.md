...
## 2024-06-12 - Form Accessibility
**Learning:** For supplementary inputs that share a visual label with another primary input (such as a range slider next to a number input), an explicit `aria-label` must be provided. This ensures full keyboard and screen reader accessibility, as `htmlFor` can only link one label to one input ID.
**Action:** Always ensure every form input has a unique `id` and a corresponding `<label htmlFor="...">`. Explicitly define an `aria-label` for supplementary inputs without their own visual labels.
