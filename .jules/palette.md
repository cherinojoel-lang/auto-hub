...
## 2024-05-18 - Accessible Character Counters for Large Textareas
**Learning:** Textareas with large limits (like 2000 chars) can cause anxiety or confusion without visual feedback, and screen reader users need polite announcements when limits are approached.
**Action:** Always pair `maxLength` on textareas with a visual counter. Ensure the counter uses `aria-live="polite"` and is linked to the input via `aria-describedby`.
