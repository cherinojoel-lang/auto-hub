...
export function getStructuredDataProduct(vehicle: any) {
  ...
  return {
    ...
    'image': [
      `https://sldwrd-my-site-wgayakj9-energievergleich.wix-vibe-site.com${vehicle.mainImage}`,
      ...(vehicle.gallery || []).slice(1, 5).map((img: string) => `https://sldwrd-my-site-wgayakj9-energievergleich.wix-vibe-site.com${img}`)
    ],
    'brand': {
      '@type': 'Brand',
      'name': vehicle.make
    ...
    'offers': {
      ...
      'url': `https://sldwrd-my-site-wgayakj9-energievergleich.wix-vibe-site.com/fahrzeugdetail/${vehicle.id}`,
      'seller': {
        '@type': 'LocalBusiness',
        'name': 'Automobile Quick',
        'address': {
