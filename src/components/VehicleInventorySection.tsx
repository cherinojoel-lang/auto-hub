...
export default function VehicleInventorySection() {
  const topVehicles = React.useMemo(() => {
    // Bolt: Replaced O(N) array iteration and allocation caused by .filter(...).slice(...)
    // with a single-pass O(1) early-break loop for improved performance and reduced memory allocations.
